		...
		$(document).ready(function() {
			...
			.then(data => {
				...
				if(status == "serverError") {
					...
					for(let i = 0; i < list.length; i++) {
						if(i >= 3) {
							htmlStr += '<div class="col-lg-4 col-md-4 col-sm-12 latest-release" style="text-align: center; padding-bottom: 45px;"><img style="width: 250px; height: 300px;" class="img-fluid" src="<%=request.getContextPath() %>' + list[i].image + '" alt=""><p style="position: absolute; bottom: 115px; left: 80px; color: white; background: red; padding: 5px 8px; letter-spacing: 1.1px;">' + list[i].status + '</p><div style="margin-top: 10px;"><a href="<%= request.getContextPath() %>/bookDetail.jsp?id=' + list[i].ISBNNo + '"><h4>' + list[i].title + '</h4></a><p></p></div></div>';
						}
						else {
							htmlStr += '<div class="col-lg-4 col-md-4 col-sm-12 latest-release" style="text-align: center; padding-bottom: 45px;"><img style="width: 250px; height: 300px;" class="img-fluid" src="<%=request.getContextPath() %>' + list[i].image + '" alt=""><p style="position: absolute; bottom: 95px; left: 80px; color: white; background: red; padding: 5px 8px; letter-spacing: 1.1px;">' + list[i].status + '</p><div style="margin-top: 10px;"><a href="<%= request.getContextPath() %>/bookDetail.jsp?id=' + list[i].ISBNNo + '"><h4>' + list[i].title + '</h4></a><p></p></div></div>';
						}
					}
